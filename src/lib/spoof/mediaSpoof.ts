...
const mediaSpoof: CustomInjectionFn = (spoofContext, CHAMELEON_SPOOF, modifiedAPIs, helpers) => {
  const MEDIA_DEVICES = {
    win: [
    ...
    ios: [
      {
        ...
        videoinput: ['Back Camera', 'Front Camera'],
      },
      {
    ...
    and: [
      {
        ...
        videoinput: ['Front facing camera', 'Back facing camera'],
      },
      {
        audioinput: 'Microphone 1',
        videoinput: ['camera2 1, facing front', 'camera2 0, facing back'],
      },
    ],
  };
  ...
  let originalEnumerateDevices = helpers && (helpers as any)._enumerateDevices;

  if (spoofContext.navigator.mediaDevices && originalEnumerateDevices) {
    if (isMobile) {
      if (platform == 'ios') {
    ...
    Object.defineProperty(spoofContext.navigator.mediaDevices, 'enumerateDevices', {
      ...
      value: () => {
        return new Promise(async resolve => {
          let devices = await originalEnumerateDevices();
          let known = {};
