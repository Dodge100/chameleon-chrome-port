...
const font: CustomInjectionFn = (spoofContext, CHAMELEON_SPOOF, modifiedAPIs, helpers) => {
  ...
  if (helpers) {
    helpers.modifyNodeFont = modifyNodeFont;
  }
    ...
    try {
      for (let i = 0; i < spoofContext.document.styleSheets.length; i++) {
        for (let j = 0; j < spoofContext.document.styleSheets[i].cssRules.length; j++) {
          ...
          let fontFamily = spoofContext.document.styleSheets[i].cssRules[j].style.fontFamily;
          if (fontFamily) {
            spoofContext.document.styleSheets[i].cssRules[j].style.fontFamily = getWhitelistFonts(fontFamily);
          }
        }
      }
    } catch (e) {
  ...
  if (helpers) {
    helpers.CHAMELEON_SPOOF_f = CHAMELEON_SPOOF_f;
  }
  ...
  modifiedAPIs.push([spoofContext.Element.prototype.setAttribute, 'setAttribute']);
};
