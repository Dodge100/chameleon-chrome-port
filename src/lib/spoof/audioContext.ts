...
const audioContext: CustomInjectionFn = (spoofContext, CHAMELEON_SPOOF, modifiedAPIs) => {
  if (spoofContext.AudioContext && spoofContext.OfflineAudioContext) {
    ...
    modifiedAPIs.push([spoofContext.AudioBuffer.prototype.getChannelData, 'getChannelData']);

    spoofContext.AudioBuffer.prototype.copyFromChannel = function(...args) {
      let c = this.getChannelData(args[1]);
    ...
    modifiedAPIs.push([spoofContext.AudioBuffer.prototype.copyFromChannel, 'copyFromChannel']);
  }
};
