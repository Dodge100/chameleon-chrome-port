...
const timezone: CustomInjectionFn = (spoofContext, CHAMELEON_SPOOF, modifiedAPIs, helpers) => {
  if (new Date()[spoofContext.CHAMELEON_SPOOF]) {
    spoofContext.Date = Date;
    return;
  }

  const ORIGINAL_DATE = spoofContext.Date;
  const ORIGINAL_INTL = (helpers && helpers.originalIntlDateTimeFormat) || spoofContext.Intl.DateTimeFormat;
